...
long FileSize(FILE *stream)         //Размер файла (в байтах)
{
    ...
    length = ftell(stream);
    fseek(stream, curpos, SEEK_SET);//Возвращаемся на исходное место
    return length;
}
...
int Zapros(FILE **a, FILE **b, FILE **log)      //Выбор режима работы
{
    int vib;
    char c;
    printf("\nВведите 1 для шифрования или 2 для дешифрования файла:");
    c=getch();
    printf("%c",c);
    if (c == '1')
    {
        *a=fopen("C:\\Users\\DiSaharID\\Desktop\\1.txt","rb");
        *b=fopen("C:\\Users\\DiSaharID\\Desktop\\2.txt","wb");
        *log=fopen("C:\\Users\\DiSaharID\\Desktop\\LogShifr.txt","wb");
        vib=1;
    }
    if (c == '2')
    {
        *a=fopen("C:\\Users\\DiSaharID\\Desktop\\2.txt","rb");
        *b=fopen("C:\\Users\\DiSaharID\\Desktop\\3.txt","wb");
        *log=fopen("C:\\Users\\DiSaharID\\Desktop\\LogRasshifr.txt","wb");
        vib=2;
    }
    return vib;
}
...
int NumbOfBloks(FILE *a, long size)             //Определение числа блоков
{
    ...
    if(size%8!=0)                               //Если число символов в файле не кратно 8, то увеличиваем число блоков
    {
        n++;
        nekratno=1;
    }
...
void ZapolnK(int *t)                //Заполнение и вывод подключа K
{
    ...
    if (rejim==2)                   //Если режим дешифрования, то
    {                               //последовательность подключей имеет следующий вид:
        if((*t>=1)&&(*t<=8))
            x=KEY[(*t-1)%8];
        else
...
void Sdvig11()                      //Сдвиг на 11 бит влево. Результат сдвига - результат функции Фейстеля F(Ai,Ki)
{
    fprintf(log,"\nF     ");
    for(int i=0;i<21;i++)
        {
